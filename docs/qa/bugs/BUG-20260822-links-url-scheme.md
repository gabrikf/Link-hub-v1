...
- **Direction for the fix (decided at triage):** `url: httpUrlSchema` in all three places, then `npm run build:schemas`. Do **not** hand-roll a scheme regex. Watch the blast radius on `linkSchema` specifically — it is the **response** schema, so tightening it means a legacy row with a non-http URL would fail serialization and turn a bad row into a 500. The dev database has zero such rows; if that is not guaranteed elsewhere, tighten the two input schemas and handle `linkSchema` with a data check first.
- **Fix commit:** —
