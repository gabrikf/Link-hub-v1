...
expected: With the policy at its strictest level, a post published through MCP names no employer in the UI, in list_my_posts, on the logged-out public profile, or in the API payload behind it
entry_points: mcp:create_post; http://localhost:5173/dashboard/settings; http://localhost:5173/profile/seed-react-frontend-003
qa_status: untested
bug_ids:
fix_status:
retest_status:
...
Free-prose notes live here and only here — including which themes the surface must be walked in, and any disclosure specifics the frontmatter cannot hold.
```
...
- **The field set is closed at 16.** The two scripts in `scripts/` parse exactly this list; adding a field means editing both of them in the same commit or the tracker view stops round-tripping. Resist the urge — theme coverage and disclosure detail belong in the body and in the evidence paths, not in new columns.
...
| 7 | `entry_points` | How a user reaches it (URL, MCP tool as `mcp:<tool>`, CLI verb) | Semicolon-separated |
...
| 13 | `evidence` | Paths to evidence for the latest verdict | Semicolon-separated paths; theme in the filename |
...
| `pass` | Walked by the assigned persona; expected observable confirmed with evidence (in every required theme) |
| `fail` | Walked; expected observable not met; `bug_ids` MUST be non-empty |
| `blocked-verify` | Only completable by a human (a real Google account, a real GitHub App install, a funded `OPENAI_API_KEY`) |
...
- `id` = `<AREA>-<slug>`. Area codes are defined once in `<qa-docs-path>/README.md` (seed set: `AUTH`, `DASH`, `RSME`, `WORK`, `LAYT`, `PROF`, `SRCH`, `POST`, `SET`, `AGNT` — see `references/qa-docs-layout.md`). Adding an area updates the README first. Slug: 2-5 kebab-case words naming the behavior from the persona's side (`signup-empty-email`, `layout-survives-reload`, `disclosure-blocks-employer`).
...
- Grandfathered counter ids from an adopted tree stay valid as-is — the content-addressed format governs new minting only; rewording an old id breaks every report, charter, and bug that cites it.
- When two scenarios test the same behavior from different angles, keep both files and cross-link via `overlaps`, naming the canonical owner first. Dedup is explicit, not silent. This happens legitimately with disclosure: `SET-disclosure-blocks-employer` (does the policy hold?) and `PROF-public-post-redacted` (does a stranger see the redaction?) describe one rule from two surfaces, and both are worth keeping.
...
- When a surface changes (a new diff touches a scenario's journey), the executor or planner resets `qa_status` to `untested` — a stale `pass` is worse than no verdict.
- **Implementers flag between cycles.** Whoever lands a user-visible behavior change (a web surface, an API route, an MCP tool, a config key, copy) — including agents completing non-QA tasks — applies the flag as part of task completion: new behavior → add `untested` scenario files; changed behavior → reset the affected files to `untested`; pure refactors declare "no user-visible change". Flag, don't retest — the next cycle's targeted tier picks up `untested` scenarios as its scope. This is what keeps the tracker alive between QA cycles.
- **A change to `packages/schemas` flags widely.** It is the contract package consumed by api, web, mcp, extractor and training, so a shape change there resets every scenario whose journey crosses that boundary — not just the one screen the author was thinking about.
...
The script mutates only the generated view; scenario files are read, never written. A file it rejects (stderr names the file and the violation) is repaired per the SKILL's error handling, then the run is repeated. The companion `explode_state.py` runs once, in the other direction, only when adopting a legacy CSV tracker (see the layout reference's adoption procedure).

Both scripts are plain Python 3 with no third-party dependencies and no knowledge of the product — they encode this schema and nothing else. Regenerate the view whenever a human wants the spreadsheet or a step needs the whole table at once; day-to-day queries are greps on `scenarios/`.
...
- **Prose statuses** — `qa_status: "Passed after retest"` makes the tracker unqueryable. The enum is `pass`; the story goes in the body and the bug file. `"pass (light only)"` is the same violation.
- **Near-duplicate enum spellings** — the enum lists above are closed; anything else is a schema violation to fix on sight.
- **Adding a column** — the 16-field list is what both scripts parse. A seventeenth field breaks the view unless both are edited in the same commit, and it is almost never what the situation actually needs.
