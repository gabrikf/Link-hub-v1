...
- No i18n at all. `<html lang="en">`, all user-visible strings hardcoded in English. **Do not report a missing `t()` call** — there is no translation layer, and inventing one is a finding against the reviewer, not the code.
