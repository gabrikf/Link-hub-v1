...
| # | Question | When to use |
|---|----------|-------------|
| 3.1 | Is this screen public or session-gated? | Every new screen — `/profile/$username` is public, everything under `/dashboard` is not |
...
- Known deliberate debt (`packages/ui`, the eslint backlog, `apps/mcp` having no tests, the `pluguins/` typo) is **never** in scope by implication
...
> Recommendation: all of them, enumerated from the zod enum in `packages/schemas/src/agent-policy/`, plus the unknown-value fallback. *(This one is not skippable — if unanswered, I assume the full enum and record the assumption in `decisions.md`.)*
>
