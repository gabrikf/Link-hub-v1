...
| `Blocked (needs human verify)` | Only completable by a human (real Google OAuth account, a real GitHub App install, a funded `OPENAI_API_KEY`); exact instructions recorded | yes — waits on a person |
...
A row walked in light mode only is **not** a `Pass` when its surface renders UI — it is `Pending` until the dark walk happens, or `Skipped` with that stated as the reason. "Passed, dark not checked" is exactly the prose status this enum exists to forbid.
...
3. **Both themes walked** — every browser row states light and dark, or names which one was not covered. A theme gap is disclosed in Final Status like any other coverage gap.
4. **Disclosure accounted for** — every charter that touched posts, settings, the public profile or the MCP surface records which disclosure edge cases it attempted, and any leak found is filed at `Data-Loss` or higher.
5. **Bugs consistent** — every `Fail`/`Fixed` row links a registered bug id; bug statuses match reality; no orphan bugs unlinked from the tracker.
6. **Fixes proven** — every fix has a commit SHA + a vitest regression test **written first and seen failing for the right reason** (or a documented replay + backlog entry); impacted and adjacent journeys re-walked in both themes; each closed bug carries its Root Cause and its GitHub issue link.
7. **Tracker written** — every settled scenario's file carries the current verdict with valid enums; every session's debrief is in the report.
8. **Evidence lean and linked** — checkpoints/failures captured with theme in the filename, paths resolve, oversized evidence pruned per the layout policy.
9. **Exit gate run** — `npm run build:schemas && node scripts/guardrails/pre-push.mjs` result recorded verbatim.
10. **Parity disclosed** — any production-parity deviation (a mocked API, a search walked without a funded `OPENAI_API_KEY`, the docker stack down, wifi-only) stated, since it qualifies every verdict.
11. **Fidelity clean** — no evaluator framing leaked into product surfaces; no runner knowledge fed to the agent persona; any violation disclosed with the re-run that replaced the tainted verdicts.
12. **Final Status states the release readiness** in one sentence a non-reader can act on, with totals by user-impact tier.
...
- **Prose statuses** — "passed but flaky on retry" is not a status; it's a `Pass` with an Issue link or a `Fail`, decided honestly. "Passed in light mode" is not a status either.
- **Batched write-back** — updating the tracker "later" is how rounds end without updating it at all.
- **Green-by-omission** — an empty report section reads as "nothing found"; it must read as "not run", explicitly. This includes the browser: a screen the driver could not reach is listed under "Not visually verified", never left silent. And a disclosure section with no entries must say "no agent-authored surface in scope", not sit empty.
