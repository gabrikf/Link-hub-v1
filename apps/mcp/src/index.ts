...
import { CraftHubApiClient } from "./api-client.js";
...
async function main(): Promise<void> {
  ...
  const client = new CraftHubApiClient(config);
  ...
  console.error(`CraftHub MCP server running on stdio (API: ${config.apiUrl})`);

  if (disclosure.degraded) {
    console.error(
      "[CraftHub MCP] Could not read your disclosure policy — assuming the " +
        "strictest level ('summary'). Create a token with the profile:read " +
        "scope in CraftHub settings to let this server read your real policy " +
        `and work history. Reason: ${disclosure.degradedReason}`,
    );
  } else {
    console.error(
      `[CraftHub MCP] Disclosure level: ${disclosure.level} (${disclosure.info.label})`,
    );
  }
}
...
main().catch((err) => {
  if (err instanceof ConfigError) {
    console.error(`\n[CraftHub MCP] Configuration error:\n${err.message}\n`);
  } else {
    const detail =
      err instanceof Error ? (err.stack ?? err.message) : String(err);
    console.error(`\n[CraftHub MCP] Fatal error:\n${detail}\n`);
  }
  process.exit(1);
});
