...
const getAvailableScopes = (
  t: TFunction,
): Array<{ value: ApiTokenScope; description: string }> => [
  { value: "posts:write", description: t("settings.scope.postsWrite") },
  { value: "posts:read", description: t("settings.scope.postsRead") },
  {
    value: "profile:read",
    ...
    description: t("settings.scope.profileRead"),
  },
  {
    ...
    // activity log.
    description: t("settings.scope.activityWrite"),
  },
];
...
export function CreateTokenDialog({
  ...
  const availableScopes = getAvailableScopes(t);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (name.trim().length === 0) {
      ...
      setNameError(t("settings.createToken.nameHelp"));
  ...
  if (created) {
    return (
      <Dialog
        ...
        title={t("settings.createToken.created")}
        ...
        <div className="space-y-4">
          <div className="flex items-start gap-2 rounded-lg border border-amber-300 bg-amber-50 p-3 text-sm text-amber-800 dark:border-amber-500/40 dark:bg-amber-500/10 dark:text-amber-200">
            <FiAlertTriangle
              ...
              aria-hidden="true"
            />
            <span>{t("settings.createToken.copyNow")}</span>
          </div>

          <div>
            <p className="mb-1 text-sm text-zinc-700 dark:text-zinc-300">
              {t("settings.createToken.yourToken")}
            </p>
          ...
          <p className="text-xs text-zinc-500 dark:text-zinc-400">
            <Trans
              i18nKey="settings.createToken.useAsEnvVar"
  ...
  return (
    <Dialog
      open={open}
      onOpenChange={onOpenChange}
      title={t("settings.createToken.title")}
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        <Input
          id={NAME_FIELD_ID}
          label={t("common.name")}
          placeholder={t("settings.createToken.namePlaceholder")}
          value={name}
        ...
        <div>
          ...
          {hasNoScopes ? (
            <p className="mt-1.5 text-sm text-red-600 dark:text-red-400">
              {t("settings.createToken.scopesRequired")}
            </p>
          ) : null}
        </div>
        ...
        <Input
          id="token-expires"
          label={t("settings.createToken.expiryOptional")}
          type="date"
          value={expiresAt}
        ...
        <div className="flex justify-end gap-2 pt-1">
          ...
          <Button
            ...
            title={
              hasNoScopes
                ? t("settings.createToken.scopesRequiredShort")
                : undefined
            }
