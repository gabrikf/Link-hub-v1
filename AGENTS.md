...
A developer-profile platform. Developers import a resume and arrange a public
...
`README.md` orients; this file is the rules.
...
change affects, lints the files you touched, checks the harness and the
locales, and prints `guardrails PASS`. It also runs on `git push` (husky) and
on the Claude Code `Stop` hook — **you are not done until it passes.**
...
run that announces what it narrowed is honest. Do not silence it.
...
- **vitest, everywhere. Never jest.** Write a test whenever the work is a
  ...
  that requires it — and check the blast radius first.
...
- **Lint is enforced, not advisory.** `npm run lint` is the syntactic layer;
  the type-aware rules ratchet, so an inherited finding passes and one you add
  fails. An inline `eslint-disable` is a workaround, not a fix.
