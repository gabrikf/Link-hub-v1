...
export default async function publicProfile({
  ...
  await goto("/seed-react-frontend-003"); // FILLED — real API. Navigate BEFORE setTheme.
  await shot("filled");

  await setTheme("dark"); // DARK is not optional (§3)
  ...
  assert(
    ...
    'error: RouteErrorState shows a "Try again" button',
  );
}
```
...
| `setTheme(theme)`                           | paints `light`/`dark`/`system` AND **proves it** — seeds `localStorage`, rewrites `GET /preferences` so the signed-in bootstrap agrees, then throws if that is not what ends up painted. Navigate first (§3) |
| `seedStoredTheme(pref)`                     | writes the local mirror and reloads, with **no claim** about what paints. For proving the database beats a stale mirror; not for taking a dark screenshot                                                    |
