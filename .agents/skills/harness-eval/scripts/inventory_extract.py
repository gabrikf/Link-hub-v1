...
# why: nested AGENTS.md are always-on in Kiro and load on demand elsewhere, so
# they are T0, not optional docs. Walked (not globbed) to prune dependency trees.
NESTED_T0_NAMES = ("AGENTS.md", "CLAUDE.md")
...
# why: every tool's path-scoped always-on rule directory. Each is a projection of
# the same harness; all of them are T0 wherever the tool that reads them is used.
RULE_GLOBS = (
    ".claude/rules/**/*.md",
    ".agents/rules/**/*.md",
    ".cursor/rules/**/*.mdc",
...
def is_nested_checkout(directory: Path) -> bool:
    """A directory that holds its own repo marker or its own root instruction file."""
    if (directory / ".git").exists():
        return True
    return (directory / "AGENTS.md").is_file() and (directory / "package.json").is_file() and (
        directory / ".claude"
    ).exists()


def discover_nested_t0(root: Path) -> list[Path]:
    """Every nested AGENTS.md / CLAUDE.md, pruning dependency and artifact trees."""
    found: set[Path] = set()
    ...
    def walk(directory: Path, depth: int) -> None:
        ...
        for child in children:
            if child.is_symlink():
                continue
            if child.is_dir():
                if child.name in NESTED_SKIP_DIRS:
                    continue
                # why: a nested checkout (git worktree, vendored clone) carries a
                # full copy of the harness. Scoring it reports one problem N times
                # and inflates the claim deck by the number of open worktrees.
                if is_nested_checkout(child):
                    continue
                walk(child, depth + 1)
            elif child.name in NESTED_T0_NAMES and directory != root:
...
def discover_t0(root: Path) -> list[Path]:
    ...
    for name in T0_NAMES:
        ...
        if p.is_file():
            found.add(p.resolve())
    for pattern in T0_GLOBS + RULE_GLOBS:
        for p in glob_files(root, pattern):
            if not is_readme(rel(root, p)):
                found.add(p.resolve())
    # resolve() collapses the symlink aliases (CLAUDE.md -> AGENTS.md,
    # .claude/skills -> .agents/skills) so one file is never two surfaces.
    found.update(discover_nested_t0(root))
    return sorted(found)
...
def manifest_dirs(root: Path) -> list[Path]:
    """Root plus every workspace that carries its own manifest.

    A monorepo splits its scripts across workspaces: `npm run dev:worker` is
    real, it just lives in `apps/api/package.json`. Reading only the root
    manifest reports every one of those as a missing script.
    """
    dirs = [root]
    ...
    def walk(directory: Path, depth: int) -> None:
        ...
        for child in children:
            if not child.is_dir() or child.is_symlink():
                continue
            if child.name in NESTED_SKIP_DIRS or child.name.startswith("."):
                continue
            if is_nested_checkout(child):
                continue
            if any((child / m).is_file() for m in MANIFEST_FILES):
                dirs.append(child)
            walk(child, depth + 1)
...
def load_manifest_commands(root: Path) -> tuple[list[str], list[str]]:
    ...
    for directory, name in (
        (d, n) for d in manifest_dirs(root) for n in MANIFEST_FILES
    ):
        p = directory / name
        if not p.is_file():
            continue
