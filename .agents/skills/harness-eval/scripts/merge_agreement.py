...
ROW_RE = re.compile(
    # why: a deck past 999 claims uses C1000+. `\d{3}` matched "C100" and then
    # failed on the trailing digit, so every four-digit row parsed as no score
    # and landed in Hold as "missing-score" — a truncated run that looks cautious.
...
def main() -> int:
    ...
    # why: the markdown report lists Ship and Hold rows but only a COUNT for
    # Review, so nothing downstream can ask "which band did claim C123 get?".
    # A no-loss proof across two runs needs exactly that, per claim.
    bands = {}
    for cid, _claim, _a, _b in ship:
        bands[cid] = "Ship"
    for cid, _claim, _a, _b in review:
        bands[cid] = "Review"
    for cid, _claim, _a, _b, reason in hold:
        bands[cid] = f"Hold:{reason}"
    (run / "07-agreement.json").write_text(
        json.dumps(
            {
                "run_dir": str(run),
                "trap_pass": trap_pass,
                "trap_misses": misses,
                "counts": {"ship": len(ship), "review": len(review), "hold": len(hold)},
                "bands": bands,
            },
            indent=2,
        )
