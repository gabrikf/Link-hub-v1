...
def main() -> int:
    ...
    # why: the ledger's no-loss gate has to ask "was this surface Keep-core?"
    # per surface path. The markdown groups by band; this is the same data keyed
    # by id and path so a diff across two runs can be mechanical.
    bands = {}
    for band, items in (("Slim", slim), ("Keep-core", keep), ("Mixed", mixed)):
        for sid, surf, _a, _b in items:
            bands[sid] = {"band": band, "path": surf.get("path"), "tier": surf.get("tier")}
    for sid, surf, _a, _b, reason in hold:
        bands[sid] = {"band": f"Hold:{reason}", "path": surf.get("path"), "tier": surf.get("tier")}
    (run / "10-usefulness-agreement.json").write_text(
        json.dumps(
            {
                "run_dir": str(run),
                "trap_pass": trap_pass,
