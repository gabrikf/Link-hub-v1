...
- **Entry:** arrive the way the persona arrives — the auth screen at `/`, the dashboard nav, a public profile link someone pasted into a chat. Never paste an internal URL the persona wouldn't have. `/<username>` is public by design and is the one URL a stranger legitimately arrives at cold.
...
|---|---|---|
| Navigate | `goto(path)` | `/`, `/dashboard`, `/dashboard/search`, `/dashboard/layout`, `/dashboard/posts`, `/dashboard/posts/review`, `/dashboard/settings`, `/<username>` |
