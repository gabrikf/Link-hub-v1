# Anti-Patterns — Failure Modes Across Five Families
...
7. Quarantine-as-cemetery + retry-as-fix — a quarantined test nobody returns to, and a retry that turns a real failure into an occasional one. The team-level pathology that converts flakiness into silent coverage loss.
...
- **Violation.** `expect(screen.getByTestId('banner')).toBeInTheDocument()` with no check on its text, attributes, or visibility — or a bare `expect(x).toBeTruthy()`.
- **Why wrong.** The test passes as long as anything renders; hides regressions in visibility, content, or state.
- **Fix.** Assert specific value, attribute, text, or state — `expect(screen.getByRole('status')).toHaveTextContent('Saved')`, not just that it exists.
...
- **Violation.** `await user.click(screen.getByRole('button', { name: 'Save' }));` — end of test, nothing checked afterward.
...
- **Violation.** `await new Promise((r) => setTimeout(r, 2000))` inside a vitest test, or `page.waitForTimeout(2000)` inside a Playwright scenario.
- **Why wrong.** Slow on fast machines, still flaky on slow CI; papers over race conditions.
- **Fix.** Wait for the condition itself — RTL's `findBy*` / `waitFor`, vitest's `expect.poll`, or Playwright's auto-retrying `await expect(locator).toBeVisible()` — and let the framework retry until satisfied.
...
- **Gate.** Will this test still pass when run alone with `it.only` (vitest) or `.only` (Playwright)?
...
- **Violation.** Database reset, logout, or file cleanup placed in `afterEach` — e.g. `afterEach(() => db.reset())` in a vitest suite.
- **Why wrong.** If the test crashes or is interrupted, cleanup never runs and the next test starts dirty.
- **Fix.** Move "must run" cleanup into `beforeEach` (re-seeding) — `beforeEach(async () => { await db.reset(); await seed(); });`.
